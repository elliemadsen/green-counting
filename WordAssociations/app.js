...
(function () {
  ...
  // Header and footer are separate siblings of <main>, so they can't size
  // themselves to the active .view (a CSS-only fit-content on each would size
  // to its OWN content, not match). Instead each view hugs its own content
  // width (see .view in style.css), and this measures the now-visible one and
  // applies that width to the header/footer's .page-col wrapper — so the
  // whole page reads as one consistently-wide, centered column no matter
  // which view (and therefore which natural width) is showing.
  function syncPageWidth() {
    const active = state.view === "corpus" ? viewCorpus
      : state.view === "cell" ? viewCell : viewSyl;
    // Two rAFs: one for layout to settle after replaceChildren(), one more
    // in case fonts/webfont metrics shift the fit-content measurement.
    requestAnimationFrame(() => requestAnimationFrame(() => {
      const w = active.getBoundingClientRect().width;
      if (!w) return;
      for (const col of document.querySelectorAll(".page-col"))
  ...
  // Re-sync on resize (e.g. narrow viewport forces the active .view to
  // shrink below its natural width) — debounced since resize fires rapidly.
  let resizeTimer = null;
  window.addEventListener("resize", () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(syncPageWidth, 120);
  });
