...
// web/bibliography/index.html, web/images/index.html), replacing all four
// page shells with password-gated versions. Everything else (data.js,
// style.css, images, ...) is copied through unchanged. docs/ mirrors web/'s
// layout flatly (docs/index.html, docs/word_associations/,
// docs/bibliography/, docs/images/) and lives at the repo root, one level
// up from web/, since that's where GitHub Pages needs it to serve from.
//
...
const SITE_SRC =__dirname; // web/
const REPO_ROOT = path.join(SITE_SRC, ".."); // repo root -- docs/ lives here, alongside web/
const DOCS = path.join(REPO_ROOT, "docs");
const PASSWORD_FILE = path.join(__dirname, "protect/.password");
...
const PAGES = [
  ...
  {
    src: path.join(SITE_SRC, "images", "index.html"),
    out: path.join(DOCS, "images", "index.html"),
    title: "Images in Architecture Syllabi", hasStyle: true,
  },
];
...
function gateHtml(title, saltB64, ivB64, dataB64, hasStyle) {
  return [
    ...
    "        border: 1px solid #ffffff; background: #474747;",
    "        color: #ffffff;",
    "      }",
    ...
    "        border: 1px solid #ffffff; background: #ffffff; color: #363636; cursor: pointer;",
...
function main() {
  ...
  copyTree(path.join(SITE_SRC, "images"), path.join(DOCS, "images"));
